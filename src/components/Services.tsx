...
const Services = () => {
  return (
    <section id="services" className="inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:32px_32px] md:bg-[size:60px_60px] relative py-24 px-4 overflow-hidden bg-transparent">
          ...
          {services.map((service, index) => (
            <ScrollReveal key={index} direction="up" delay={index * 0.1}>
              ...
              >
                ...
                <div className={`absolute -inset-[2px] bg-gradient-to-br ${service.accent} rounded-[32px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-[4px]`} />
                <div className={`absolute -inset-[1px] bg-gradient-to-br ${service.accent} rounded-[32px] opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
                
                <Card className="h-full relative bg-white dark:bg-slate-950/80 backdrop-blur-3xl border-slate-200 dark:border-white/5 rounded-[30px] overflow-hidden transition-all duration-500 group-hover:shadow-[0_20px_50px_rgba(8,112,184,0.2)]">
                  {/* Dynamic Radial Hover Background */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${service.accent} opacity-0 group-hover:opacity-[0.07] transition-opacity duration-700`} />
                  
                  <CardHeader className="p-8 relative z-10">
                    <div className="flex justify-between items-start mb-6">
                      <motion.div 
                        whileHover={{ rotate: 10, scale: 1.1 }}
                        className={`p-4 rounded-2xl bg-gradient-to-br ${service.accent} text-white shadow-lg shadow-cyan-500/20`}
                      >
                    ...
                    <CardTitle className="text-2xl font-black dark:text-white mb-3 group-hover:text-cyan-500 transition-colors duration-300">
                      {service.title}
                    </CardTitle>
                    
                    <div className="flex flex-wrap gap-2 mb-4">
                      {service.tags.map(tag => (
                        <span key={tag} className="text-[10px] font-black px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/10 text-slate-500 dark:text-slate-400 uppercase tracking-tighter border border-transparent group-hover:border-white/10 transition-colors">
                          #{tag}
                        </span>
                      ))}
                    </div>

                    <CardDescription className="text-sm leading-relaxed text-slate-600 dark:text-slate-400 font-bold">
                      {service.description}
                    </CardDescription>
                  </CardHeader>

                  <CardContent className="px-8 pb-8 flex items-center gap-2 relative z-10">
                    <span className="h-1.5 w-1.5 rounded-full bg-cyan-500 animate-pulse shadow-[0_0_8px_rgba(6,182,212,0.8)]" />
                    <span className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.2em]">Live Solution</span>
                  </CardContent>

                  {/* Animated Background Shape */}
                  <div className={`absolute -right-8 -bottom-8 w-32 h-32 bg-gradient-to-br ${service.accent} opacity-0 group-hover:opacity-20 rounded-full transition-all duration-1000 blur-3xl group-hover:scale-150`} />
                </Card>
              </motion.div>
            </ScrollReveal>
          ))}
